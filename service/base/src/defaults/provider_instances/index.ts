...
import temporamDefault from "./temporam-default.json" with { type: "json" };
...
export function createDefaultProviderInstances(now: Date): ProviderInstance[] {
  return [
    ...
    hydrateInstance(toProviderInstance(temporamDefault, "provider_instances.temporam-default"), now),
  ];
}
