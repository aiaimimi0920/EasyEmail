...
describe("mail account HTTP resources", () => {
  it("supports authenticated create/list/get/update/disable/delete without raw secrets", async () => {
    ...
    try {
      ...
      const createdPayload = await created.json() as { account: { id: string; version: number; credentialRefs: unknown[] } };
      expect(createdPayload.account).toMatchObject({ version: 1, credentialRefs: [{ secretKey: "ref:v1:http-work" }] });
