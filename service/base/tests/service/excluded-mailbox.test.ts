...
describe("mailbox excluded domains and addresses", () => {
  ...
  it("skips a strongly blacklisted mailbox-domain provider on later automatic selection", async () => {
    const service = createBootstrappedEasyEmailService({
      providerTypes: [
        createProviderType("mailtm", "Mail.tm"),
        createProviderType("m2u", "MailToYou"),
  ...
  it("temporarily skips a weak mailbox-domain outcome during its policy cooldown", async () => {
    ...
    const cooled = await service.openMailbox({
      hostId: "demo-host-2",
      provisionMode: "reuse-only",
      bindingMode: "shared-instance",
      providerStrategyModeId: "available-first",
    ...
    const recovered = await service.openMailbox({
      hostId: "demo-host-3",
      provisionMode: "reuse-only",
      bindingMode: "shared-instance",
      providerStrategyModeId: "available-first",
      providerGroupSelections: ["m2u", "mailtm"],
    }, new Date("2026-06-04T00:12:00.000Z"));

    expect(cooled.instance.providerTypeKey).toBe("mailtm");
    expect(cooled.session.emailAddress).toBe("good@mail.tm");
    expect(recovered.instance.providerTypeKey).toBe("m2u");
    expect(recovered.session.emailAddress).toBe("bad@risk.test");
  });

  it("does not reintroduce a structured blacklist provider during open fallback", async () => {
    const service = createBootstrappedEasyEmailService({
      providerTypes: [
        createProviderType("mailtm", "Mail.tm"),
        createProviderType("m2u", "MailToYou"),
    ...
    service.reportMailboxOutcome({
      ...
      attribution: {
        ...
        providerTypeKey: "mailtm",
        domain: "mail.tm",
        emailAddress: "blacklisted@mail.tm",
      },
      policy: {
        avoidInCurrentAttempt: true,
        globalBlacklist: true,
